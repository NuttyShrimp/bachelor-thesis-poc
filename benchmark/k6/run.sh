...
for key in "${!operations[@]}"; do
  ...
  for value in "${values[@]}"; do
    ...
    K6_PROMETHEUS_RW_SERVER_URL=http://localhost:9090/api/v1/write K6_PROMETHEUS_RW_TREND_AS_NATIVE_HISTOGRAM=true K6_PROMETHEUS_RW_PUSH_INTERVAL=1s K6_PROMETHEUS_RW_TREND_STATS="p(95),p(99),min,max,avg" OPERATION=$key SCENARIO=$value k6 run -o experimental-prometheus-rw ./script.ts --tag testid="$RUNTIME-$key-$value";
    ...
    docker compose restart php-fpm php-octane
